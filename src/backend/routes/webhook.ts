    ...
    if (body.object) {
        if (
            ...
            try {
                ...
                if (lead.isAiPaused) {
                    console.log(`[WEBHOOK] AI is paused for lead ${lead.phone}. Skipping auto reply.`);
                    return res.sendStatus(200);
                }

                // Check for Human Handoff Keywords
                const lowercaseMsg = msgBody.toLowerCase();
                const handoffKeywords = ["agent", "owner", "human"];
                const isHandoffRequest = handoffKeywords.some(keyword => lowercaseMsg.includes(keyword));

                if (isHandoffRequest) {
                    console.log(`[WEBHOOK] Handoff requested by ${lead.phone}. Pausing AI.`);
                    
                    await prisma.lead.update({
                        where: { id: lead.id },
                        data: { isAiPaused: true, status: "HANDOFF" }
                    });

                    const handoffMessage = "Sure, I am transferring you to a human agent now. The owner/staff will get back to you shortly. 📞";
                    
                    if (business.autoReplyEnabled) {
                        await waService.sendTextMessage(from, handoffMessage);
                ...
                if (matchedAutomation) {
                    ...
                    // Extract lead details (Name, Requirement) in background
                    aiService.extractLeadInfo(fullContext + `\nCustomer: ${msgBody}\nAgent: ${replyMessage}`).then(async (extracted) => {
                        if (extracted.name || extracted.requirement) {
                            const updateData: any = {};
                            if (extracted.name && !lead.name) {
                                updateData.name = extracted.name;
                            }
                            if (extracted.requirement && !lead.customerInterest) {
                                updateData.customerInterest = extracted.requirement;
                                updateData.leadType = "INTERESTED";
                            }
                            if (Object.keys(updateData).length > 0) {
                                ...
                                console.log(`[AUTO_CAPTURE] Updated lead ${lead.phone}:`, updateData);
                            }
                        }
                    }).catch(err => {
                        console.error("[AUTO_CAPTURE] Error:", err.message || err);
                    });
